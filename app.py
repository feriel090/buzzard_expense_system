...
def dashboard_dept():
    ...
    expenses = Expense.query.filter_by(department=current_user.department).order_by(Expense.id.desc()).all()
    ...
    reports = db.session.query(
        ...
        Expense.status=="Approved",
        Expense.department==current_user.department
    ).group_by(Expense.category).all()

    return render_template('dashboard_dept.html', expenses=expenses, reports=reports)
...
def dashboard_it():
    ...
    users = User.query.all()  # IT can see all users
    return render_template('dashboard_it.html', users=users)

# ===== Reset Password for IT Staff =====
@app.route('/it/reset_password', methods=['POST'])
@login_required
def reset_password_it():
    if current_user.role != 'it':
        flash("Access Denied", "error")
        return redirect(url_for('login'))
...
# ===== Submit New Expense (for Head and Finance) =====
@app.route('/new_expense', methods=['GET','POST'])
@login_required
def new_expense():
    if current_user.role not in ['finance', 'head', 'admin']:
        flash("Access Denied", "error")
        return redirect(url_for('login'))
